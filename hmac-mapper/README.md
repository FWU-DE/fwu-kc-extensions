...
5. Local sub identifier which gives the unique identifier whose value is to be used during the pseudonymized sub generation. It should be the attributes of user like id, username, etc.(By default username is used)

## How it's working

The *HMAC Pairwise subject with static sectorIdentifier* mapper always hashes the user's ID with a host and the configured salt. This value is then mapped to the `sub` attribute in the returned token.
Meaning, when two clients have the same host and salt, they will return the same `sub` for a given user ID.

Sector Identifier URI is mandatory and therefore redirect URIs are never be used by mapper *HMAC Pairwise subject with static sectorIdentifier*.
...
7. `POST` a login request to the `desight` client (with same sector identifier URI, but different salt than `formand`)
