...
public class RestrainingAuthenticationFlowContext implements AuthenticationFlowContext
{

    private final AuthenticationFlowContext delegate;
    private final List<String>              restrainingIdPs;

    public RestrainingAuthenticationFlowContext(AuthenticationFlowContext delegate, List<String> restrainingIdPs)
    {
        this.delegate = delegate;
        this.restrainingIdPs = restrainingIdPs;
    }
    ...
    {
        return delegate.getTopLevelFlow();
    }
    ...
    public void success()
    {
        UserModel user = delegate.getUser();
        if (user != null) {
            boolean linkedToRestrainingIdp = delegate.getSession().users()
                .getFederatedIdentitiesStream(delegate.getRealm(), user)
                .map(FederatedIdentityModel::getIdentityProvider)
                .anyMatch(restrainingIdPs::contains);
            boolean hasPairwiseSub = user.getFirstAttribute("pairwiseSub") != null;
            if (linkedToRestrainingIdp || hasPairwiseSub) {
                delegate.attempted();
                return;
            }
        }
    ...
    {
        delegate.success(credentialType);
    }
